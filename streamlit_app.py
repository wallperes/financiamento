...
def buscar_indices_bc(mes_inicial, meses_total):
    """
    Busca índices INCC (189) e IPCA (433) do Banco Central
    """
    try:
        ...
        df_combined = pd.DataFrame({
            'incc': df_incc['value'] / 100 if not df_incc.empty else 0,
            'ipca': df_ipca['value'] / 100 if not df_ipca.empty else 0
        }).fillna(0)
