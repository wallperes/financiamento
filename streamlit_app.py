...
def main():
    ...
    if 'df_resultado' in st.session_state:
        ...
        # Exibir e baixar índices se disponíveis
        if st.session_state.df_indices is not None and not st.session_state.df_indices.empty:
            st.subheader("Índices de Correção (Banco Central)")
            st.dataframe(st.session_state.df_indices)
            
            output_indices = io.BytesIO()
            with pd.ExcelWriter(output_indices, engine='xlsxwriter') as writer:
                st.session_state.df_indices.to_excel(writer, sheet_name='Índices BC')
            st.download_button(
                label="📥 Baixar índices do BC",
                data=output_indices.getvalue(),
                file_name='indices_correcao.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            )
