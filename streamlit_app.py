...
def main():
    ...
    with col1:
        if st.button("1. Simular com Médias", type="primary", use_container_width=True, help="Usa as taxas médias estimadas para todo o período. Ideal para uma projeção geral."):
            st.session_state.df_resultado = simular_financiamento(params.copy())

    with col2:
        if st.button("2. Simular Híbrido (BC + Médias)", use_container_width=True, help="Usa os dados reais do BC e completa o restante com as médias. Bom para projeções baseadas na realidade."):
            num_parcelas_entrada = params.get('num_parcelas_entrada', 0)
