...
def calcular_correcao(saldo, mes, fase, params, valores_reais):
    ...
    if valores_reais is not None:
        if mes in valores_reais:
            ...
            if fase in ['Entrada','Pré', 'Carência'] and idx.get('incc') is not None: return saldo * idx['incc']
            elif fase == 'Pós' and idx.get('ipca') is not None: return saldo * idx['ipca']
...
def simular_financiamento(params, valores_reais=None):
    ...
    historico.append({'Mês/Data': f"Assinatura [{data_assinatura.strftime('%m/%Y')}]", 'Fase': 'Assinatura', 'Saldo Devedor': saldo_devedor, 'Parcela Total': amortizacao_assinatura, 'Amortização Base': amortizacao_assinatura, 'Correção INCC ou IPCA diluída (R$)': 0, 'Taxa de Juros (%)': 0, 'Juros (R$)': 0, 'Ajuste INCC (R$)': 0, 'Ajuste IPCA (R$)': 0})

    # >>> ALTERAÇÃO CRÍTICA: Lógica da Carência com diluição <<<
    ...
    for i in range(meses_carencia):
        ...
        historico.append({'Mês/Data': f"Gerou Correção [{data_corrente_carencia.strftime('%m/%Y')}]", 'Fase': 'Carência', 'Saldo Devedor': saldo_devedor, 'Parcela Total': 0, 'Amortização Base': 0, 'Correção INCC ou IPCA diluída (R$)': 0, 'Taxa de Juros (%)': 0, 'Juros (R$)': 0, 'Ajuste INCC (R$)': correcao_mes_carencia, 'Ajuste IPCA (R$)': 0})
    ...
    try:
        ...
        for mes in range(1, meses_total + 1):
            ...
            if data_referencia_str in dados_por_data:
                ...
                incc_val, ipca_val = valores['incc'], valores['ipca']
                if incc_val is not None or ipca_val is not None: ultimo_mes_com_dado = mes
...
def criar_parametros():
    ...
    params = {'mes_assinatura': st.sidebar.text_input("Mês da assinatura (MM/AAAA)", value="04/2025", help="Mês de assinatura do contrato."), 'mes_primeira_parcela': st.sidebar.text_input("Mês da 1ª parcela (MM/AAAA)", value="05/2025", help="Mês de vencimento do primeiro boleto."), 'valor_total_imovel': st.sidebar.number_input("Valor total do imóvel", value=455750.0, step=1000.0, format="%.2f"), 'valor_entrada': st.sidebar.number_input("Valor total da entrada", value=22270.54, step=100.0, format="%.2f")}
    params['tipo_pagamento_entrada'] = st.sidebar.selectbox("Como a entrada é paga?", ['Parcelada', 'Paga no ato'], help="'Paga no ato': O valor total da entrada é pago na assinatura. 'Parcelada': A entrada é dividida em boletos a partir do mês da 1ª parcela.")
    if params['tipo_pagamento_entrada'] == 'Parcelada':
        ...
        if params['num_parcelas_entrada'] > 0: params['entrada_mensal'] = params['valor_entrada'] / params['num_parcelas_entrada']
        else: params['entrada_mensal'] = 0
    else:
        params['num_parcelas_entrada'], params['entrada_mensal'] = 0, 0
    st.sidebar.subheader("Parâmetros de Correção")
    params['inicio_correcao'] = st.sidebar.number_input(label="Aplicar correção a partir de qual parcela?", min_value=1, value=1, step=1, help="Define o número da parcela a partir da qual a correção (INCC/IPCA) começa. Ex: Se a entrada tem 3x, '1' inicia a correção na 1ª da entrada; '4' inicia na 1ª pós-entrada.")
    ...
    with col1: params['meses_pre'] = col1.number_input("Meses pré-chaves", value=17, min_value=0, step=1)
    with col2: params['meses_pos'] = col2.number_input("Meses pós-chaves", value=100, min_value=0, step=1)
    col3, col4 = st.sidebar.columns(2)
    with col3: params['parcelas_mensais_pre'] = col3.number_input("Valor parcela pré (R$)", value=3983.38, format="%.2f")
    with col4: params['valor_amortizacao_pos'] = col4.number_input("Valor parcela pós (R$)", value=3104.62, format="%.2f")
    ...
    for i in range(4):
        ...
        with col_sem1: mes_sem = col_sem1.number_input(f"Mês da {i+1}ª semestral", min_value=0, value=6*(i+1) if i<2 else 0, key=f"sem_mes_{i}")
        with col_sem2: valor_sem = col_sem2.number_input(f"Valor {i+1} (R$)", min_value=0.0, value=6000.0 if i<2 else 0.0, key=f"sem_val_{i}", format="%.2f")
    ...
    with col_anu1: mes_anu = col_anu1.number_input("Mês da anual", min_value=0, value=17, key="anu_mes")
    with col_anu2: valor_anu = col_anu2.number_input("Valor anual (R$)", min_value=0.0, value=43300.0, key="anu_val", format="%.2f")
...
def main():
    ...
    with col2:
        if st.button("Simular com Valores Reais do BC", use_container_width=True, help="Busca os índices reais no Banco Central e os aplica onde houver dados. Usa as médias para o período futuro."):
            ...
            if ultimo_mes > 0: st.info(f"Dados reais do BC encontrados e aplicados até a parcela {ultimo_mes}. O restante da simulação usará as médias estimadas.")
            st.session_state.df_resultado = simular_financiamento(params.copy(), valores_reais)
    with col3:
        limite_correcao = st.number_input("Aplicar correção só até a parcela:", min_value=1, value=params['meses_pre'] + params.get('num_parcelas_entrada', 0), step=1, help="Simula um cenário onde a correção (INCC/IPCA) para de ser aplicada após um certo número de parcelas.")
    ...
    if not st.session_state.df_resultado.empty:
        ...
        if 'df_export' in st.session_state:
            ...
            st.download_button(label="💾 Baixar Planilha (XLSX)", data=output.getvalue(), file_name='simulacao_financiamento.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', use_container_width=True)
